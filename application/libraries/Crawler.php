...
class Crawler{
    ...
    public function get_images(){
    	$images = [];
    	$count = 0;
    	if(!$page_keywords = $this->dom->find('img')){
    		return false;
    	}
    	foreach ($page_keywords as $img) {
    ...
    /**
	 * Get content from loaded URL
	 *
	 * @return string
	 */
    public function get_contents_by_class($class=''){
    	if($class == ''){
    		return false;
    	}
    	if(!$page_keywords = $this->dom->find('.'.$class, 0)->innertext()){
    		return false;
    	}
    ...
    public function get_contents_by_id($id=''){
    	...
    	if(!$page_keywords = $this->dom->find('#'.$id, 0)->innertext()){
    		return false;
    	}
